package practice.codility.lessons.l1;
 ...
 */
class BinaryGap {
    // My solution: ko cần thiết phải chuyển đổi sang cơ số 2 sau đó mới tính binary gap,
    // có thể tính luôn trong lúc chia: vừa chia liên tục số đó cho 2 vừa đếm
    public int solution(int n) {
        int r = 0;      // remain
        int curBg = 0;  // count current binary gap
        int maxBg = 0;  // count max binary gap
        ...
        while (n > 0) {
            ...
            if (r == 1) {
                startCnt = true;
                if (curBg > 0) {
                    // gặp số 1 lần 2, kết thúc count curBg
                    if (curBg > maxBg)
                        maxBg = curBg;
                    curBg = 0;
                }
            } else {
                // sau khi gặp số 1 lần 1, bắt đầu count curBg
                if (startCnt)
                    curBg++;
            }
        }
